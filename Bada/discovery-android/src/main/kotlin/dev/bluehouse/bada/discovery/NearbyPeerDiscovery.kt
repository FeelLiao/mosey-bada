...
public class NearbyPeerDiscovery internal constructor(
    ...
    private val moseyEvents: Flow<MoseyPeerEvent>,
) {
    public constructor(context: Context) : this(
        ...
        moseyEvents = MoseyPeerScanner(context.applicationContext).scan(),
    )
            ...
            val moseyJob =
                launch {
                    moseyEvents.collect { event ->
                        when (event) {
                            is MoseyPeerEvent.Resolved ->
                                aggregator.onMoseyResolved(event.device).forEach { trySend(it).isSuccess }
                            is MoseyPeerEvent.Lost ->
                                aggregator.onMoseyLost(event.endpointId).forEach { trySend(it).isSuccess }
                        }
                    }
                }
    ...
    public companion object {
        ...
        internal fun forTesting(
            ...
            moseyEvents: Flow<MoseyPeerEvent> = emptyFlow(),
        ): NearbyPeerDiscovery =
            NearbyPeerDiscovery(
                lanEvents = lanEvents,
                bleEvents = bleEvents,
                bluetoothEvents = bluetoothEvents,
                moseyEvents = moseyEvents,
            )
    }
}
...
private class PeerAggregator {
    ...
    private val moseyIndex: MutableMap<String, String> = HashMap()

    fun onMoseyResolved(device: dev.bluehouse.bada.discovery.medium.AppleDevice): List<NearbyPeerEvent> {
        val peerId = moseyIndex[device.endpointId] ?: "airdrop:${device.endpointId}"
        val state = peersById.getOrPut(peerId) { MutablePeer(stableId = peerId) }
        val before = state.toPeerOrNull()
        state.airDropEndpointId = device.endpointId
        state.airDropServiceName = device.deviceName
        state.airDropDisplayName = device.deviceName
        state.airDropHost = device.ipv6 ?: device.ipv4
        state.airDropPort = device.port
        moseyIndex[device.endpointId] = state.stableId
        return changeEvents(before, state.toPeerOrNull())
    }

    fun onMoseyLost(endpointId: String): List<NearbyPeerEvent> {
        val peerId = moseyIndex.remove(endpointId) ?: return emptyList()
        val state = peersById[peerId] ?: return emptyList()
        val before = state.toPeerOrNull()
        state.airDropEndpointId = null
        state.airDropServiceName = null
        state.airDropDisplayName = null
        state.airDropHost = null
        state.airDropPort = null
    ...
    private fun finalizeChange(
        ...
        state.airDropEndpointId?.let(moseyIndex::remove)
        state.lanRouteKey?.let(lanRouteIndex::remove)
    ...
    private fun filterReason(peer: NearbyPeer): String =
        buildList {
            ...
            if (peer.airDropEndpoint == null) add("airdrop-missing")
...
private data class MutablePeer(
    ...
    var airDropEndpointId: String? = null,
    var airDropServiceName: String? = null,
    var airDropDisplayName: String? = null,
    var airDropHost: String? = null,
    var airDropPort: Int? = null,
) {
    fun toPeerOrNull(): NearbyPeer? {
        ...
        val airDrop =
            if (airDropHost != null && airDropPort != null && airDropServiceName != null) {
                NearbyPeer.AirDropEndpoint(
                    serviceName = airDropServiceName!!,
                    displayName = airDropDisplayName ?: airDropServiceName!!,
                    host = airDropHost!!,
                    port = airDropPort!!,
                )
            } else {
        ...
        if (lan == null && bluetooth == null && ble == null && airDrop == null) return null
        return NearbyPeer(
            stableId = stableId,
            endpointId = endpointId,
