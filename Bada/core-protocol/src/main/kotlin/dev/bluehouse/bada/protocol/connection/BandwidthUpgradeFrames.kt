...
public object BandwidthUpgradeFrames {
    ...
    public fun decodeCredentials(info: UpgradePathInfo): UpgradePathCredentials? {
        ...
        return when (medium) {
            ...
            //
            // MOSEY (AWDL/AirDrop) — credentials are never decoded from
            // Quick Share wire frames. The mosey transport is set up
            // directly via NDK Binder calls to mosey_server, not through
            // the bandwidth-upgrade negotiation. We return Generic here
            // to satisfy exhaustiveness; the actual credentials are
            // carried by [UpgradePathCredentials.Mosey] which is
            // constructed programmatically in [MoseyMediumProvider].
            Medium.BLE_L2CAP,
            Medium.BLE,
            Medium.WEB_RTC,
            Medium.MOSEY,
    ...
    private fun encodeCredentials(credentials: UpgradePathCredentials): UpgradePathInfo {
        ...
        when (credentials) {
            ...
            // BLE L2CAP CoC (#52) — handled above via early return.
            is UpgradePathCredentials.BleL2cap -> Unit
            // Mosey (AWDL/AirDrop) — not serialized onto the Quick Share
            // wire protocol. The mosey transport is set up directly via
            // NDK Binder calls to mosey_server, not through the
            // bandwidth-upgrade negotiation frames. No proto fields to
            // set here.
            is UpgradePathCredentials.Mosey -> Unit
        }
