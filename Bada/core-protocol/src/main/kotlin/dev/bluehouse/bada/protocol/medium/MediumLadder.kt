 ...
 *  3. **Mosey (AWDL)** — Apple Wireless Direct Link; direct AWDL radio
 *     path via mosey_server + wonder.ko (OnePlus/Pixel ODM extension).
 *     Ranked alongside Wi-Fi Direct because it provides a similar
 *     peer-to-peer Wi-Fi data path without a router.
 *  4. **Wi-Fi Hotspot** — same bandwidth ceiling as LAN, but requires
 *     standing up a soft-AP (slow on cold start).
 *  5. **Wi-Fi LAN** — the already-open discovery transport.
 *  6. **WebRTC** — internet-relayed; only relevant when both peers
 *     have connectivity but no shared LAN.
 *  7. **BLE L2CAP** — meaningful throughput on the LE radio (≈ 1 Mbps
 *     on BT 5+); preferred over RFCOMM for the file-transfer phase.
 *  8. **Bluetooth (RFCOMM)** — slowest of the bulk-transfer mediums but
 *     extremely widely supported; the universal fallback.
 *  9. **BLE (GATT)** — discovery-only in practice; left in the ladder
