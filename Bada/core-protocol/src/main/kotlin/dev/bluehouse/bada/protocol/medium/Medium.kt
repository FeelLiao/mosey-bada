...
public enum class Medium(
    ...
    /**
     * AWDL / Apple Wireless Direct Link — the mosey/AirDrop transport.
     * Wire number 13.
     *
     * Unlike every other medium in this enum, MOSEY is not part of the
     * stock Quick Share wire protocol. It is an ODM-level extension
     * shipped by OnePlus (and Pixel) that bridges Quick Share to Apple's
     * AirDrop ecosystem using the native `mosey_server` daemon.
     *
     * Bada interacts with it through the NDK Binder interface exposed
     * by `mosey_server` (`com.google.android.moseyservice.IMoseyService`),
     * not through the protobuf-based upgrade path. The wire number 13
     * matches `AWDL` in both `ConnectionRequestFrame.Medium` and
     * `UpgradePathInfo.Medium` but the actual transport setup happens
     * entirely in-kernel (wonder.ko + nl80211).
     */
    MOSEY(13),
    ;
    ...
    public companion object {
         ...
         * — none of which are in scope for this Android port.
         *
         * Note: `AWDL` (13) resolves to [MOSEY] and is in scope via
         * the mosey AirDrop integration (see `docs/research/mosey-airdrop-integration.md`).
         */
