...
public sealed interface UpgradePathCredentials {
    ...
    /**
     * Mosey (AWDL/AirDrop) credentials — the country code for regulatory
     * channel selection.
     *
     * Unlike other mediums, Mosey does NOT carry network-layer connection
     * parameters (no IP, port, SSID, or MAC address). The AWDL radio I/O
     * is handled entirely by the native `mosey_server` daemon through
     * `wonder.ko` / nl80211. Bada's role is limited to telling the daemon
     * which regulatory domain to use.
     *
     * These credentials are never serialized onto the Quick Share wire
     * protocol. They are passed directly to [MoseyBinderClient.update]
     * via the NDK Binder interface.
     *
     * @param countryCode ISO 3166-1 alpha-2 country code (e.g. "US", "CN", "JP").
     *   Determines which 5 GHz channels the AWDL radio is permitted to use.
     */
    public data class Mosey(
        val countryCode: String,
    ) : UpgradePathCredentials {
        override val medium: Medium = Medium.MOSEY

        init {
            require(countryCode.length == COUNTRY_CODE_LENGTH) {
                "Country code must be exactly $COUNTRY_CODE_LENGTH characters, got '$countryCode'"
            }
            require(countryCode.all { it.isUpperCase() }) {
                "Country code must be uppercase ISO 3166-1 alpha-2, got '$countryCode'"
            }
        }

        public companion object {
            /** Length of an ISO 3166-1 alpha-2 country code. */
            public const val COUNTRY_CODE_LENGTH: Int = 2
        }
    }
}
