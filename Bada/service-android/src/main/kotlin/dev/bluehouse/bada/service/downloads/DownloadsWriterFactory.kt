...
public object DownloadsWriterFactory {
    public fun saveAirDropFile(
        ...
        val handle = DownloadsWriter(chooseEnvironment(context.applicationContext))
            .beginWrite(fileName, mimeType, parentFolder)
        return try {
            input.copyTo(handle.outputStream)
            handle.commit()
            handle.displayName
        } catch (t: Throwable) {
            handle.discard()
